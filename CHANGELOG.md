...
### Added
- BFV : optional additional plaintext types (NewPlaintextZQ, NewPlaintextZT or NewPlaintextMul) optimized for different operation. See bfv/plaintext.go for the full details.
- RING : GenNTTPrimes now takes the Nth primitive root as input rather than logN.

### Changed
- BFV : QiMul is now automatically generated by the evaluator and does not need to be provided by the user.
