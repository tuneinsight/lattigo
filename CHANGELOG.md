...
### Changed
- BFV : plaintext must not be created using NewPlaintextZQ, NewPlaintextZT or NewPlaintextMul. See bfv/plaintext.go for the full details.
- BFV : QiMul is now automatically generated by the evaluator
- RING : GenNTTPrimes now takes the Nth primitive root as input rather than logN.

### Removed
- BFV : removed QiMul from the parameters
