...
type Evaluator interface {
	...
	EvaluatePolyVector(input interface{}, pols []*Polynomial, encoder Encoder, slotIndex map[int][]int, targetScale rlwe.Scale) (ctOut *Ciphertext, err error)
...
func (eval *evaluator) matchScaleThenEvaluateInPlace(el0, el1, elOut *Ciphertext, evaluate func(int, *ring.Poly, uint64, *ring.Poly)) {
	...
	r0, r1, _ := eval.matchScalesBinary(el0.Scale().(*Scale).Value, el1.Scale().(*Scale).Value)
...
func (eval *evaluator) AddScalar(ctIn *Ciphertext, scalar uint64, ctOut *Ciphertext) {
	...
	if !ctIn.Scale().Equal(1) {
		scalar = ring.BRed(scalar, ctIn.Scale().(*Scale).Value, ringT.Modulus[0], ringT.BredParams[0])
	}
...
func (eval *evaluator) MulScalarAndAdd(ctIn *Ciphertext, scalar uint64, ctOut *Ciphertext) {
	...
	if !ctIn.Scale().Equal(ctOut.Scale()) {
		ringT := eval.params.RingT()
		ratio := ring.ModExp(ctIn.Scale().(*Scale).Value, ringT.Modulus[0]-2, ringT.Modulus[0])
		ratio = ring.BRed(ratio, ctOut.Scale().(*Scale).Value, ringT.Modulus[0], ringT.BredParams[0])
...
func (eval *evaluator) rescaleOriginal(ctIn, ctOut *Ciphertext) (err error) {
	...
	ctOut.Scale().Div(ringQ.Modulus[level])
...
func (eval *evaluator) Relinearize(ctIn *Ciphertext, ctOut *Ciphertext) {
	...
	eval.GadgetProduct(level, eval.buffQ[0], eval.Rlk.Keys[0].GadgetCiphertext, rlwe.Ciphertext{Value: []*ring.Poly{eval.BuffQP[1].Q, eval.BuffQP[2].Q}})
	...
	for deg := ctIn.Degree() - 1; deg > 1; deg-- {
		...
		eval.GadgetProduct(level, eval.buffQ[0], eval.Rlk.Keys[deg-2].GadgetCiphertext, rlwe.Ciphertext{Value: []*ring.Poly{eval.BuffQP[1].Q, eval.BuffQP[2].Q}})
		ringQ.MulScalarAndAddLvl(level, eval.BuffQP[1].Q, T, ctOut.Value[0])
		ringQ.MulScalarAndAddLvl(level, eval.BuffQP[2].Q, T, ctOut.Value[1])
	}
...
func (eval *evaluator) SwitchKeys(ctIn *Ciphertext, swk *rlwe.SwitchingKey, ctOut *Ciphertext) {
	...
	eval.GadgetProduct(level, eval.buffQ[0], swk.GadgetCiphertext, rlwe.Ciphertext{Value: []*ring.Poly{eval.BuffQP[1].Q, eval.BuffQP[2].Q}})
...
func (eval *evaluator) Automorphism(ctIn *Ciphertext, galEl uint64, ctOut *Ciphertext) {
	...
	ringQ.MulScalarBigintLvl(level, ctIn.Value[1], eval.tInvModQ[level], eval.buffQ[0])
	eval.GadgetProduct(level, eval.buffQ[0], rtk.GadgetCiphertext, rlwe.Ciphertext{Value: []*ring.Poly{eval.BuffQP[1].Q, eval.BuffQP[2].Q}})
...
func (eval *evaluator) AutomorphismHoisted(level int, ctIn *Ciphertext, c1DecompQP []ringqp.Poly, galEl uint64, ctOut *Ciphertext) {
	...
	eval.GadgetProductHoisted(level, c1DecompQP, rtk.GadgetCiphertext, rlwe.Ciphertext{Value: []*ring.Poly{eval.BuffQP[0].Q, eval.BuffQP[1].Q}})
...
func (eval *evaluator) MatchScalesAndLevel(ct0, ct1 *Ciphertext) {
	...
	r0, r1, _ := eval.matchScalesBinary(ct0.Scale().(*Scale).Value, ct1.Scale().(*Scale).Value)
...
func (eval *evaluator) Merge(ctIn map[int]*Ciphertext) (ctOut *Ciphertext) {
	...
	for _, ct := range ciphertextslist {
		...
		if ct != nil {
			if scale1 := ct.Scale().(*Scale).Value; scale1 != scale0 {
