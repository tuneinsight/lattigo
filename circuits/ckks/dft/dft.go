...
import (
	...
	ltfloat "github.com/tuneinsight/lattigo/v5/circuits/ckks/linear_transformation"
...
type DFTMatrix struct {
	DFTMatrixLiteral
	Matrices []ltfloat.LinearTransformation
}
...
type DFTEvaluator struct {
	*ckks.Evaluator
	*ltfloat.LinearTransformationEvaluator
...
func NewDFTEvaluator(params ckks.Parameters, eval *ckks.Evaluator) *DFTEvaluator {
	dfteval := new(DFTEvaluator)
	dfteval.Evaluator = eval
	dfteval.LinearTransformationEvaluator = ltfloat.NewLinearTransformationEvaluator(eval)
...
func (eval *DFTEvaluator) dft(ctIn *rlwe.Ciphertext, matrices []ltfloat.LinearTransformation, opOut *rlwe.Ciphertext) (err error) {
...
func (d DFTMatrixLiteral) GenMatrices(LogN int, prec uint) (plainVector []ltfloat.Diagonals[*bignum.Complex]) {
	...
	plainVector = make([]ltfloat.Diagonals[*bignum.Complex], maxDepth)
