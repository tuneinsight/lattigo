...
func GenMinimaxCompositePolynomialForSign(prec uint, logalpha, logerr int, deg []int) {
	...
	fmt.Printf("{\n")
	for i := range coeffs {
		PrettyPrintCoefficients(decimals, coeffs[i], true, false)
	}
	fmt.Printf("},\n")
}
...
func GenMinimaxCompositePolynomialForStep(prec uint, logalpha, logerr int, deg []int) {
	...
	coeffsLast := coeffs[len(coeffs)-1]

	two := new(big.Float).SetInt64(2)

	// Changes the last poly to scale the output by 0.5 and add 0.5
	for j := range coeffsLast {
		coeffsLast[j].Quo(coeffsLast[j], two)
	}

	coeffsLast[0].Add(coeffsLast[0], new(big.Float).SetFloat64(0.5))
...
func PrettyPrintCoefficients(decimals int, coeffs []*big.Float, odd, even bool) {
	fmt.Printf("{")
	for i, c := range coeffs {
		if (i&1 == 1 && odd) || (i&1 == 0 && even) {
...
func TestMinimaxApprox(t *testing.T) {
	...
	deg := []int{15, 15, 15, 17, 31, 31, 31, 31}

	GenMinimaxCompositePolynomialForSign(prec, logalpha, logerr, deg)
}
*/
