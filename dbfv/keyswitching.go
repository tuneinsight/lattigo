...
func (cks *CKSProtocol) genShareDelta(skDelta *ring.Poly, ct *bfv.Ciphertext, shareOut CKSShare) {
	...
	ringP := cks.context.ringP
	contextQP := cks.context.ringQP

	ringQ.NTT(ct.Value()[1], cks.tmpNtt)
	ringQ.MulCoeffsMontgomery(cks.tmpNtt, skDelta, shareOut.Poly)
	ringQ.MulScalarBigint(shareOut.Poly, ringP.ModulusBigint, shareOut.Poly)

	ringQ.InvNTT(shareOut.Poly, shareOut.Poly)

	cks.gaussianSampler.ReadLvl(uint64(len(contextQP.Modulus)-1), cks.tmpNtt)
