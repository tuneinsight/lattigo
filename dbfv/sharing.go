...
func (s2e *S2EProtocol) GenShare(sk *rlwe.SecretKey, crp drlwe.CRP, secretShare *rlwe.AdditiveShare, c0ShareOut *drlwe.CKSShare) {
	s2e.encoder.ScaleUp(&bfv.PlaintextRingT{Plaintext: &rlwe.Plaintext{Value: &secretShare.Value}}, s2e.tmpPlaintext)
	s2e.CKSProtocol.GenShare(s2e.zero, sk, &rlwe.Ciphertext{Value: []*ring.Poly{c0ShareOut.Value, crp.Get(0).Q}}, c0ShareOut)
...
func (s2e *S2EProtocol) GetEncryption(c0Agg *drlwe.CKSShare, c1 drlwe.CRP, ctOut *bfv.Ciphertext) {
	if ctOut.Degree() != 1 {
		panic("ctOut must have degree 1.")
	}
	ctOut.Value[0].Copy(c0Agg.Value)
	ctOut.Value[1].Copy(c1.Get(0).Q)
}
