...
func (rfp *MaskedTransformProtocol) AllocateShare() (*MaskedTransformShare, drlwe.RefreshCRP) {
	level := len(rfp.ringQ.Modulus) - 1
	return &MaskedTransformShare{*rfp.e2s.AllocateShare(level), *rfp.s2e.AllocateShare(level)}, rfp.ringQ.NewPolyLvl(level)
}
