...
func (rotkg *RotKGProtocol) AllocateShareRotColLeft() (r RotKGShareRotColLeft) {
	r = make([]*ring.Poly, rotkg.ckksContext.Beta())
	for i := uint64(0); i < rotkg.ckksContext.Beta(); i++ {
		r[i] = rotkg.ckksContext.ContextKeys().NewPoly()
	}
	return
}
...
func NewRotKGProtocol(ckksContext *ckks.CkksContext) (rotkg *RotKGProtocol) {

	rotkg = new(RotKGProtocol)
	rotkg.ckksContext = ckksContext

	rotkg.gaussianSampler = ckksContext.GaussianSampler()

	rotkg.rot_col_L = make(map[uint64][][2]*ring.Poly)
	rotkg.rot_col_R = make(map[uint64][][2]*ring.Poly)

	rotkg.polypool = ckksContext.ContextKeys().NewPoly()
...
func (rotkg *RotKGProtocol) genShare(sk *ring.Poly, galEl uint64, crp []*ring.Poly, evakey []*ring.Poly) {
	...
	contextKeys := rotkg.ckksContext.ContextKeys()

	ring.PermuteNTT(sk, galEl, rotkg.polypool)
	contextKeys.Sub(rotkg.polypool, sk, rotkg.polypool)

	for _, pj := range rotkg.ckksContext.KeySwitchPrimes() {
	...
	for i := uint64(0); i < rotkg.ckksContext.Beta(); i++ {
		...
		for j := uint64(0); j < rotkg.ckksContext.Alpha(); j++ {

			index = i*rotkg.ckksContext.Alpha() + j
...
func (rotkg *RotKGProtocol) StoreRotColLeft(share RotKGShareRotColLeft, k uint64, crp []*ring.Poly) {
	...
	k &= ((1 << (rotkg.ckksContext.LogN() - 1)) - 1)

	rotkg.rot_col_L[k] = make([][2]*ring.Poly, rotkg.ckksContext.Beta())

	for i := uint64(0); i < rotkg.ckksContext.Beta(); i++ {
		rotkg.rot_col_L[k][i][0] = share[i].CopyNew()
		rotkg.rot_col_L[k][i][1] = crp[i].CopyNew()
		rotkg.ckksContext.ContextKeys().MForm(rotkg.rot_col_L[k][i][1], rotkg.rot_col_L[k][i][1])
	}
}
...
func (rotkg *RotKGProtocol) StoreRotColRight(share RotKGShareRotColRight, k uint64, crp []*ring.Poly) {
	...
	rotkg.rot_col_R[k] = make([][2]*ring.Poly, rotkg.ckksContext.Beta())

	for i := uint64(0); i < rotkg.ckksContext.Beta(); i++ {
		rotkg.rot_col_R[k][i][0] = share[i].CopyNew()
		rotkg.rot_col_R[k][i][1] = crp[i].CopyNew()
		rotkg.ckksContext.ContextKeys().MForm(rotkg.rot_col_R[k][i][1], rotkg.rot_col_R[k][i][1])
	}
}

func (rotkg *RotKGProtocol) StoreConjugate(share RotKGShareConjugate, crp []*ring.Poly) {

	rotkg.rot_row = make([][2]*ring.Poly, rotkg.ckksContext.Beta())

	for i := uint64(0); i < rotkg.ckksContext.Beta(); i++ {
		rotkg.rot_row[i][0] = share[i].CopyNew()
		rotkg.rot_row[i][1] = crp[i].CopyNew()
		rotkg.ckksContext.ContextKeys().MForm(rotkg.rot_row[i][1], rotkg.rot_row[i][1])
	}
}
