...
func GetMinimumLevelForBootstrapping(lambda int, scale float64, nParties int, moduli []uint64) (minLevel int, logBound uint, ok bool) {
	logBound = uint(lambda + int(math.Ceil(math.Log2(scale))))
