...
1. Setup  
    ...
    1. _[OPTIONAL]_ Threshold Secret-Key Generation
...
4. Output phase  (Decryption)
    1. _[OPTIONAL]_ Threshold Combine Phase  
...
- Each party generates a `drlwe.ShamirPolynomial` by using the `ThresholdizerProtocol.GenShamirPolynomial` method, then generates a share of type `drlwe.ShamirSecretShare` for each of the other parties' `ShamirPublicKey` by using the `ThresholdizerProtocol.GenShamirSecretShare`.
...
- Each party aggregates the all `ShamirSecretShare`s it receives using the `ThresholdizerProtocol.AggregateShares` method.
...
#### 4.1 _[OPTIONAL]_Combining Step
