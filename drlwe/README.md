# DRLWE
The DRLWE package implements Multiparty Homomorphic Encryption (MHE) primitives based on the ring-learning-with-errors.
It provides generic interfaces for the local steps of a the MHE-based Secure Multiparty Computation (MHE-MPC) protocol that are common between all the RLWE distributed schemes implemented in Lattigo (e.g., collective key generation).
The `dbfv` and `dckks` packages import the `drlwe` and provide scheme-specific functionalities (e.g., collective key-switching).
...
The protocol enables a group of N _input parties_ to compute a joint function over their private inputs under encryption and to provide an _receiver party_ with the result.
...
1. Setup Phase
    ...
    4. Collective Public Relinearization-Key Generation  
    5. Collective Public Rotation-Key Generation
2. Evaluation Phase
   1. Input (Encryption)
...
Similarly to LSSS-based MPC protocols such as SPDZ, the setup phase does on depend on the input and can be pre-computed.
