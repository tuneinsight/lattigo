...
func (cks *CKSProtocol) GenShare(skInput, skOutput *rlwe.SecretKey, ct rlwe.Ciphertext, shareOut *CKSShare) {
	...
	if !el.IsNTT {
		...
		cks.gaussianSampler.ReadLvl(uint64(len(cks.ringQP.Modulus)-1), cks.tmpQP)
...
func (cks *CKSProtocol) AggregateShares(share1, share2, shareOut *CKSShare) {
	cks.ringQ.AddLvl(uint64(len(share1.Value.Coeffs)-1), share1.Value, share2.Value, shareOut.Value)
}
