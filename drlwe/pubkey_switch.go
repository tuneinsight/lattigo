...
func NewPCKSProtocol(params rlwe.Parameters, sigmaSmudging float64) *PCKSProtocol {
	...
	pcks.gaussianSampler = ring.NewGaussianSampler(prng, pcks.ringQP, sigmaSmudging, uint64(6*sigmaSmudging))
...
func (pcks *PCKSProtocol) AggregateShares(share1, share2, shareOut *PCKSShare) {
	level1, level2 := uint64(len(share1.Value[0].Coeffs))-1, uint64(len(share2.Value[0].Coeffs))-1
	if level1 != level2 {
