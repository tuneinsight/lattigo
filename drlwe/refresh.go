...
func (share *RefreshShare) Encode(p []byte) (n int, err error) {
	if n, err = share.EncToShareShare.Encode(p[n:]); err != nil {
		return
	}
	var inc int
	inc, err = share.ShareToEncShare.Encode(p[n:])
	return n + inc, err
}
...
func (share *RefreshShare) Decode(p []byte) (n int, err error) {
	if n, err = share.EncToShareShare.Decode(p[n:]); err != nil {
		return
	}
	var inc int
	inc, err = share.ShareToEncShare.Decode(p[n:])
	return n + inc, err
}
