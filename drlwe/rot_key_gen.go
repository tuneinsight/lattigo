...
func NewRTGProtocol(params rlwe.Parameters) *RTGProtocol {
	...
	rtg.gaussianSampler = ring.NewGaussianSampler(prng, rtg.ringQP, params.Sigma(), uint64(6*params.Sigma()))
...
func (rtg *RTGProtocol) GenShare(sk *rlwe.SecretKey, galEl uint64, crp []*ring.Poly, shareOut *RTGShare) {
	...
	for i := uint64(0); i < rtg.params.Beta(); i++ {
		...
		for j := uint64(0); j < rtg.params.Alpha(); j++ {
