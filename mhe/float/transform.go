...
func (mltp MaskedLinearTransformationProtocol) WithParams(paramsOut float.Parameters) MaskedLinearTransformationProtocol {
	...
	return MaskedLinearTransformationProtocol{
		...
		mask:         mask,
		encoder:      float.NewEncoder(paramsOut, mltp.prec),
	}
}
...
// The function is called with a vector of *Complex modulo float.Parameters.Slots() as input, and must write
...
// paramsIn: the float.Parameters of the ciphertext before the protocol.
// paramsOut: the float.Parameters of the ciphertext after the protocol.
...
func NewMaskedLinearTransformationProtocol(paramsIn, paramsOut float.Parameters, prec uint, noise ring.DistributionParameters) (mltp MaskedLinearTransformationProtocol, err error) {
	...
	mltp.encoder = float.NewEncoder(paramsOut, prec)
