...
func GetTestName(opname string, parties int, params float.Parameters) string {
...
func genTestParams(params float.Parameters, NParties int) (tc *testContext, err error) {
	...
	tc.evaluator = float.NewEvaluator(tc.params, nil)
...
func testEncToShareProtocol(tc *testContext, t *testing.T) {
	...
	t.Run(GetTestName("EncToShareProtocol", tc.NParties, params), func(t *testing.T) {
		...
		pt := float.NewPlaintext(params, ciphertext.Level())
		pt.IsNTT = false
		pt.Scale = ciphertext.Scale
		tc.ringQ.AtLevel(pt.Level()).SetCoefficientsBigint(rec.Value, pt.Value)

		float.VerifyTestVectors(params, tc.encoder, nil, coeffs, pt, params.LogDefaultScale(), 0, *printPrecisionStats, t)
		...
		ctRec := float.NewCiphertext(params, 1, params.MaxLevel())
		ctRec.Scale = params.DefaultScale()
		P[0].s2e.GetEncryption(P[0].publicShareS2E, crp, ctRec)

		float.VerifyTestVectors(params, tc.encoder, tc.decryptorSk0, coeffs, ctRec, params.LogDefaultScale(), 0, *printPrecisionStats, t)
	})
}
...
func testRefresh(tc *testContext, t *testing.T) {
	...
	t.Run(GetTestName("N->2N/Transform=nil", tc.NParties, paramsIn), func(t *testing.T) {
		...
		var paramsOut float.Parameters
		var err error
		paramsOut, err = float.NewParametersFromLiteral(float.ParametersLiteral{
...
func testRefreshParameterized(tc *testContext, paramsOut float.Parameters, skOut []*rlwe.SecretKey, transform *MaskedLinearTransformationFunc, t *testing.T) {
	...
	float.VerifyTestVectors(paramsOut, float.NewEncoder(paramsOut), rlwe.NewDecryptor(paramsOut, skIdealOut), coeffs, ciphertext, paramsOut.LogDefaultScale(), 0, *printPrecisionStats, t)
}
