...
func GetTestName(opname string, p integer.Parameters, parties int) string {
...
func gentestContext(nParties int, params integer.Parameters) (tc *testContext, err error) {
	...
	tc.evaluator = integer.NewEvaluator(tc.params, nil)
...
func testEncToShares(tc *testContext, t *testing.T) {
	...
	t.Run(GetTestName("ShareToEncProtocol", tc.params, tc.NParties), func(t *testing.T) {
		...
		ctRec := integer.NewCiphertext(tc.params, 1, tc.params.MaxLevel())
...
func testRefreshAndTransformSwitchParams(tc *testContext, t *testing.T) {
	...
	t.Run(GetTestName("RefreshAndTransformSwitchparams", tc.params, tc.NParties), func(t *testing.T) {
		...
		var paramsOut integer.Parameters
		var err error
		paramsOut, err = integer.NewParametersFromLiteral(integer.ParametersLiteral{
			LogN:             paramsIn.LogN(),
		...
		integer.NewEncoder(paramsOut).Decode(dec.DecryptNew(ciphertext), coeffsHave)
...
func newTestVectors(tc *testContext, encryptor *rlwe.Encryptor, t *testing.T) (coeffs []uint64, plaintext *rlwe.Plaintext, ciphertext *rlwe.Ciphertext) {
	...
	plaintext = integer.NewPlaintext(tc.params, tc.params.MaxLevel())
