...
func (rfp *MaskedTransformProtocol) SampleCRP(level int, crs sampling.PRNG) multi_party.KeySwitchCRP {
	return rfp.s2e.SampleCRP(level, crs)
}

// AllocateShare allocates the shares of the PermuteProtocol
func (rfp MaskedTransformProtocol) AllocateShare(levelDecrypt, levelRecrypt int) multi_party.RefreshShare {
	return multi_party.RefreshShare{EncToShareShare: rfp.e2s.AllocateShare(levelDecrypt), ShareToEncShare: rfp.s2e.AllocateShare(levelRecrypt)}
}
...
func (rfp MaskedTransformProtocol) GenShare(skIn, skOut *rlwe.SecretKey, ct *rlwe.Ciphertext, crs multi_party.KeySwitchCRP, transform *MaskedTransformFunc, shareOut *multi_party.RefreshShare) (err error) {
	...
	rfp.e2s.GenShare(skIn, ct, &multi_party.AdditiveShare{Value: rfp.tmpMask}, &shareOut.EncToShareShare)
	mask := rfp.tmpMask
	if transform != nil {
	...
	return rfp.s2e.GenShare(skOut, crs, multi_party.AdditiveShare{Value: mask}, &shareOut.ShareToEncShare)
}

// AggregateShares sums share1 and share2 on shareOut.
func (rfp MaskedTransformProtocol) AggregateShares(share1, share2 multi_party.RefreshShare, shareOut *multi_party.RefreshShare) (err error) {
...
func (rfp MaskedTransformProtocol) Transform(ct *rlwe.Ciphertext, transform *MaskedTransformFunc, crs multi_party.KeySwitchCRP, share multi_party.RefreshShare, ciphertextOut *rlwe.Ciphertext) (err error) {
	...
	rfp.e2s.GetShare(nil, share.EncToShareShare, ct, &multi_party.AdditiveShare{Value: rfp.tmpMask}) // tmpMask RingT(m - sum M_i)
	mask := rfp.tmpMask
	if transform != nil {
		coeffs := make([]uint64, len(mask.Coeffs[0]))
	...
	return rfp.s2e.GetEncryption(multi_party.KeySwitchShare{Value: ciphertextOut.Value[0]}, crs, ciphertextOut)
}
