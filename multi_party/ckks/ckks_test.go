...
func TestMULTI_PARTYFloat(t *testing.T) {
...
func testEncToShareProtocol(tc *testContext, t *testing.T) {
	...
	t.Run(GetTestName("EncToShareProtocol", tc.NParties, params), func(t *testing.T) {
		...
		type Party struct {
			...
			sk             *rlwe.SecretKey
			publicShareE2S multi_party.KeySwitchShare
			publicShareS2E multi_party.KeySwitchShare
			secretShare    multi_party.AdditiveShareBigint
		}
...
func testRefreshParameterized(tc *testContext, paramsOut ckks.Parameters, skOut []*rlwe.SecretKey, transform *MaskedLinearTransformationFunc, t *testing.T) {
	...
	type Party struct {
		...
		sIn   *rlwe.SecretKey
		sOut  *rlwe.SecretKey
		share multi_party.RefreshShare
	}
