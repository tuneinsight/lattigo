...
func (mltp MaskedLinearTransformationProtocol) AllocateShare(levelDecrypt, levelRecrypt int) multi_party.RefreshShare {
	return multi_party.RefreshShare{EncToShareShare: mltp.e2s.AllocateShare(levelDecrypt), ShareToEncShare: mltp.s2e.AllocateShare(levelRecrypt)}
}
...
func (mltp MaskedLinearTransformationProtocol) SampleCRP(level int, crs sampling.PRNG) multi_party.KeySwitchCRP {
...
// value for logBound.
func (mltp MaskedLinearTransformationProtocol) GenShare(skIn, skOut *rlwe.SecretKey, logBound uint, ct *rlwe.Ciphertext, crs multi_party.KeySwitchCRP, transform *MaskedLinearTransformationFunc, shareOut *multi_party.RefreshShare) (err error) {
	...
	if err = mltp.e2s.GenShare(skIn, logBound, ct, &multi_party.AdditiveShareBigint{Value: mask}, &shareOut.EncToShareShare); err != nil {
		return
	}
	...
	return mltp.s2e.GenShare(skOut, crs, ct.MetaData, multi_party.AdditiveShareBigint{Value: mask}, &shareOut.ShareToEncShare)
}

// AggregateShares sums share1 and share2 on shareOut.
func (mltp MaskedLinearTransformationProtocol) AggregateShares(share1, share2, shareOut *multi_party.RefreshShare) (err error) {
...
func (mltp MaskedLinearTransformationProtocol) Transform(ct *rlwe.Ciphertext, transform *MaskedLinearTransformationFunc, crs multi_party.KeySwitchCRP, share multi_party.RefreshShare, ciphertextOut *rlwe.Ciphertext) (err error) {
	...
	mltp.e2s.GetShare(nil, share.EncToShareShare, ct, &multi_party.AdditiveShareBigint{Value: mask})
	...
	if err = mltp.s2e.GetEncryption(multi_party.KeySwitchShare{Value: ciphertextOut.Value[0]}, crs, ciphertextOut); err != nil {
		return
	}
