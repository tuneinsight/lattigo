...
func (context *Ring) Add(p1, p2, p3 *Poly) {
...
func (context *Ring) AddLvl(level uint64, p1, p2, p3 *Poly) {
...
func (context *Ring) AddNoMod(p1, p2, p3 *Poly) {
...
func (context *Ring) AddNoModLvl(level uint64, p1, p2, p3 *Poly) {
	for i := uint64(0); i < level+1; i++ {
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		for j := uint64(0); j < context.N; j++ {
			p3tmp[j] = p1tmp[j] + p2tmp[j]
...
func (context *Ring) Sub(p1, p2, p3 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) SubLvl(level uint64, p1, p2, p3 *Poly) {
	for i := uint64(0); i < level+1; i++ {
		qi := context.Modulus[i]
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) SubNoMod(p1, p2, p3 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		for j := uint64(0); j < context.N; j++ {
...
// for the moduli q_0 up to q_level and writes the result on p3.
func (context *Ring) SubNoModLvl(level uint64, p1, p2, p3 *Poly) {
	for i := uint64(0); i < level+1; i++ {
		qi := context.Modulus[i]
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) Neg(p1, p2 *Poly) {
	for i, qi := range context.Modulus {
...
func (context *Ring) NegLvl(level uint64, p1, p2 *Poly) {
	for i := uint64(0); i < level+1; i++ {
		qi := context.Modulus[i]
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) Reduce(p1, p2 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
...
func (context *Ring) ReduceLvl(level uint64, p1, p2 *Poly) {
	for i := uint64(0); i < level+1; i++ {
		qi := context.Modulus[i]
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
		bredParams := context.bredParams[i]
...
func (context *Ring) Mod(p1 *Poly, m uint64, p2 *Poly) {
	params := BRedParams(m)
	for i := range context.Modulus {
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) AND(p1 *Poly, m uint64, p2 *Poly) {
	for i := range context.Modulus {
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
		for j := uint64(0); j < context.N; j++ {
			p2tmp[j] = p1tmp[j] & m
...
func (context *Ring) OR(p1 *Poly, m uint64, p2 *Poly) {
	for i := range context.Modulus {
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
		for j := uint64(0); j < context.N; j++ {
			p2tmp[j] = p1tmp[j] | m
...
func (context *Ring) XOR(p1 *Poly, m uint64, p2 *Poly) {
	for i := range context.Modulus {
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
		for j := uint64(0); j < context.N; j++ {
			p2tmp[j] = p1tmp[j] ^ m
...
func (context *Ring) MulCoeffs(p1, p2, p3 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		bredParams := context.bredParams[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) MulCoeffsAndAdd(p1, p2, p3 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		bredParams := context.bredParams[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) MulCoeffsAndAddNoMod(p1, p2, p3 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		bredParams := context.bredParams[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) MulCoeffsMontgomery(p1, p2, p3 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
...
func (context *Ring) MulCoeffsMontgomeryLvl(level uint64, p1, p2, p3 *Poly) {
	for i := uint64(0); i < level+1; i++ {
		qi := context.Modulus[i]
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		mredParams := context.mredParams[i]
...
func (context *Ring) MulCoeffsMontgomeryAndAdd(p1, p2, p3 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		mredParams := context.mredParams[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) MulCoeffsMontgomeryAndAddLvl(level uint64, p1, p2, p3 *Poly) {
	for i := uint64(0); i < level+1; i++ {
		qi := context.Modulus[i]
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		mredParams := context.mredParams[i]
...
func (context *Ring) MulCoeffsMontgomeryAndAddNoMod(p1, p2, p3 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		mredParams := context.mredParams[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) MulCoeffsMontgomeryAndAddNoModLvl(level uint64, p1, p2, p3 *Poly) {
	for i := uint64(0); i < level+1; i++ {
		qi := context.Modulus[i]
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		mredParams := context.mredParams[i]
...
func (context *Ring) MulCoeffsMontgomeryConstantAndAddNoModLvl(level uint64, p1, p2, p3 *Poly) {
	for i := uint64(0); i < level+1; i++ {
		qi := context.Modulus[i]
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		mredParams := context.mredParams[i]
...
func (context *Ring) MulCoeffsMontgomeryAndSub(p1, p2, p3 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		mredParams := context.mredParams[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) MulCoeffsMontgomeryAndSubNoMod(p1, p2, p3 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		mredParams := context.mredParams[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) MulCoeffsConstant(p1, p2, p3 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		bredParams := context.bredParams[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) MulCoeffsMontgomeryConstant(p1, p2, p3 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp, p3tmp := p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i]
		mredParams := context.mredParams[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) MulPoly(p1, p2, p3 *Poly) {
...
func (context *Ring) MulPolyMontgomery(p1, p2, p3 *Poly) {
...
func (context *Ring) MulPolyNaive(p1, p2, p3 *Poly) {
...
func (context *Ring) MulPolyNaiveMontgomery(p1, p2, p3 *Poly) {
...
func (context *Ring) Exp(p1 *Poly, e uint64, p2 *Poly) {
...
func (context *Ring) AddScalar(p1 *Poly, scalar uint64, p2 *Poly) {
	for i, Qi := range context.Modulus {
...
func (context *Ring) AddScalarBigint(p1 *Poly, scalar *big.Int, p2 *Poly) {
	tmp := new(big.Int)
	for i, Qi := range context.Modulus {
...
func (context *Ring) SubScalar(p1 *Poly, scalar uint64, p2 *Poly) {
	for i, Qi := range context.Modulus {
		p1tmp, p2tmp := p1.Coeffs[i], p1.Coeffs[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) SubScalarBigint(p1 *Poly, scalar *big.Int, p2 *Poly) {
	tmp := new(big.Int)
	for i, Qi := range context.Modulus {
		scalarQi := tmp.Mod(scalar, NewUint(Qi)).Uint64()
		p1tmp, p2tmp := p1.Coeffs[i], p1.Coeffs[i]
...
func (context *Ring) MulScalar(p1 *Poly, scalar uint64, p2 *Poly) {
	for i, Qi := range context.Modulus {
...
func (context *Ring) MulScalarLvl(level uint64, p1 *Poly, scalar uint64, p2 *Poly) {
	for i := uint64(0); i < level+1; i++ {
		Qi := context.Modulus[i]
		scalarMont := MForm(BRedAdd(scalar, Qi, context.bredParams[i]), Qi, context.bredParams[i])
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
...
func (context *Ring) MulScalarBigint(p1 *Poly, scalar *big.Int, p2 *Poly) {
...
//for the moduli q_0 up to q_level and writes the result on p2.
func (context *Ring) MulScalarBigintLvl(level uint64, p1 *Poly, scalar *big.Int, p2 *Poly) {
	scalarQi := new(big.Int)
	for i := uint64(0); i < level+1; i++ {
		Qi := context.Modulus[i]
		scalarQi.Mod(scalar, NewUint(Qi))
...
func (context *Ring) Shift(p1 *Poly, n uint64, p2 *Poly) {
...
func (context *Ring) MForm(p1, p2 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
		bredParams := context.bredParams[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) MFormLvl(level uint64, p1, p2 *Poly) {
	for i := uint64(0); i < level+1; i++ {
		qi := context.Modulus[i]
		bredParams := context.bredParams[i]
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
...
func (context *Ring) InvMForm(p1, p2 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
		mredParams := context.mredParams[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) MulByPow2New(p1 *Poly, pow2 uint64) (p2 *Poly) {
	p2 = context.NewPoly()
...
func (context *Ring) MulByPow2(p1 *Poly, pow2 uint64, p2 *Poly) {
	context.MForm(p1, p2)
	for i, Qi := range context.Modulus {
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
		mredParams := context.mredParams[i]
...
func (context *Ring) MulByPow2Lvl(level uint64, p1 *Poly, pow2 uint64, p2 *Poly) {
...
func (context *Ring) MultByMonomialNew(p1 *Poly, monomialDeg uint64) (p2 *Poly) {
	p2 = context.NewPoly()
...
func (context *Ring) MultByMonomial(p1 *Poly, monomialDeg uint64, p2 *Poly) {
...
func (context *Ring) MulByVectorMontgomery(p1 *Poly, vector []uint64, p2 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
		mredParams := context.mredParams[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) MulByVectorMontgomeryAndAddNoMod(p1 *Poly, vector []uint64, p2 *Poly) {
	for i, qi := range context.Modulus {
		p1tmp, p2tmp := p1.Coeffs[i], p2.Coeffs[i]
		mredParams := context.mredParams[i]
		for j := uint64(0); j < context.N; j++ {
...
func (context *Ring) BitReverse(p1, p2 *Poly) {
...
func (context *Ring) Rotate(p1 *Poly, n uint64, p2 *Poly) {
