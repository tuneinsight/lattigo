...
func (pol *Poly) Degree() uint64 {
	return uint64(len(pol.Coeffs[0]))
}

// LenModuli returns the current number of moduli.
func (pol *Poly) LenModuli() uint64 {
	return uint64(len(pol.Coeffs))
}
...
func (pol *Poly) GetDataLen32(WithMetadata bool) (cnt uint64) {
	cnt = uint64((pol.LenModuli() * pol.Degree()) << 2)
...
func (pol *Poly) WriteCoeffs(data []byte) (uint64, error) {
	...
	cnt, err := WriteCoeffsTo(0, pol.Degree(), pol.LenModuli(), pol.Coeffs, data)
	return cnt, err
...
func (pol *Poly) GetDataLen(WithMetadata bool) (cnt uint64) {
	cnt = uint64((pol.LenModuli() * pol.Degree()) << 3)
