...
// ReadNTT samples a polynomial into pol, then operates the NTT transform on it.
func (ts *TernarySampler) ReadNTT(pol *Poly) {
	ts.sample(pol)
	ts.context.NTT(pol, pol)
}

// ReadNTTNew allocates and samples a polynomial, then operates the NTT transform before returning it.
func (ts *TernarySampler) ReadNTTNew() (pol *Poly) {
	pol = ts.context.NewPoly()
	ts.sample(pol)
	ts.context.NTT(pol, pol)
	return pol
