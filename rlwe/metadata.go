...
// EncodeMetadata encodes the object into a binary form on a preallocated slice of bytes
// and returns the number of bytes written.
func (m MetaData) EncodeMetadata(p []byte) (n int, err error) {

	if len(p) < m.BinarySize() {
		return 0, fmt.Errorf("cannot encode metadata: len(p) is too small")
	}

	if n, err = m.PlaintextScale.EncodeScale(p[n:]); err != nil {
		return 0, err
	}
...
// DecodeMetadata decodes a slice of bytes generated by EncodeMetadata
// on the object and returns the number of bytes read.
func (m *MetaData) DecodeMetadata(p []byte) (n int, err error) {

	if len(p) < m.BinarySize() {
		return 0, fmt.Errorf("canoot Decode: len(p) is too small")
	}

	if n, err = m.PlaintextScale.DecodeScale(p[n:]); err != nil {
		return
	}
