...
func NewPlaintext(params Parameters, level int) (pt *Plaintext) {
	return &Plaintext{Value: ring.NewPoly(params.N(), level), Scale: params.DefaultScale()}
}

// NewPlaintextNTT creates a new Plaintext at level `level` from the parameters
// with coefficients NTT flag set to true.
func NewPlaintextNTT(params Parameters, level int) (pt *Plaintext) {
...
// NewPlaintextNTTAtLevelFromPoly constructs a new Plaintext at a specific level
// where the message is set to the passed poly. No checks are performed on poly and
// the returned Plaintext will share its backing array of coefficients.
// Sets the NTT flag of the returned plaintext to true.
func NewPlaintextNTTAtLevelFromPoly(level int, poly *ring.Poly) (pt *Plaintext) {
	pt = NewPlaintextAtLevelFromPoly(level, poly)
	pt.Value.IsNTT = true
	return
}
