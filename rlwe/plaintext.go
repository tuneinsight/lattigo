...
func (pt Plaintext) Copy(other *Plaintext) {
	pt.Operand.Copy(&other.Operand)
	pt.Value = other.Operand.Value[0]
}
...
func (pt Plaintext) Equal(other *Plaintext) bool {
	return pt.Operand.Equal(&other.Operand) && pt.Value.Equal(&other.Value)
}
...
func (pt *Plaintext) ReadFrom(r io.Reader) (n int64, err error) {
	if n, err = pt.Operand.ReadFrom(r); err != nil {
		return
	}

	pt.Value = pt.Operand.Value[0]
	return
}
...
func (pt *Plaintext) UnmarshalBinary(p []byte) (err error) {
	if err = pt.Operand.UnmarshalBinary(p); err != nil {
		return
	}
	pt.Value = pt.Operand.Value[0]
	return
